...
def trainer(snapshot_manager: SnapshotManager,
    ...
    iteration = snapshot_manager.restore(model, optimizer) #last train epoch
    ...
    for i in range(iteration + 1, iterations + 1):
        model.train()
        x, x_mask, y, y_mask = map(to_tensor, next(training_set)) #mask: lookback nan 
        optimizer.zero_grad()
        ...
        if i%300 == 0:
            print(f"Iteration {i}/{iterations}, Training Loss: {training_loss.item():.4f}")
